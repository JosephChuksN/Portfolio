import React from "react";



function About(){
     
    return(
        <div id="AboutSec" className="p-4 scroll-mt-24 w-full text-gray-50 mb-32">
            <h1 className="text-center text-3xl mb-9">ABOUT</h1>
            <p className="tracking-tighter  text-justify">
            A frontend developer developer and a graduate of the University of maiduguri 
            based in Abuja Nigeria with 3 years experience in frontend development, 
            I am an enthusiast in learning new technologies relating to web development 
            development always curious about solving problems relating to my field.
            </p>
        </div>
    )
}
export default About;